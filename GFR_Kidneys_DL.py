...
def main():
    ...
    for epoch in range(config['training']['epoch'], config['training']['n_epochs']):
        ...
        for i, batch in enumerate(dataloader):
            ...
            predicted_curves = np.zeros((50, 150))
            predicted_params = output.clone().detach().cpu().numpy()
            for i in range (predicted_curves.shape[0]):
                predicted_curves[i] = ToftsModel(predicted_params[i], t, AIF)

            predicted_tensor = torch.tensor(predicted_curves, device="cuda:0")
            loss = criterion(predicted_tensor, curves)

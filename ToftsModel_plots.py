...
axs[0, 0].plot(np.linspace(0, 5, 100), np.linspace(0, 5, 100), '--', color='r', label="Unity line")
...
axs[0, 1].plot(np.linspace(0, 6, 100), np.linspace(0, 6, 100), '--', color='r', label="Unity line")
...
axs[1, 0].plot(np.linspace(0.2, 0.8, 100), np.linspace(0.2, 0.8, 100), '--', color='r', label="Unity line")
axs[1, 0].set_title(f'Plasma volume fraction $v_p$ of r$^2$ score {np.round(r3, 3)}')
axs[1, 0].set_ylabel("Predicted parameter")
axs[1, 0].set_xlabel("Input parameter")
axs[1, 0].legend()

axs[1, 1].scatter(input_params[:,3], predicted_params[:,3])
axs[1, 1].plot(np.linspace(1, 3.5, 100), np.linspace(1, 3.5, 100), '--', color='r', label="Unity line")
axs[1, 1].set_title(f'Time delay $\Delta$ (s) of r$^2$ score {np.round(r4, 3)}')
axs[1, 1].set_xlabel("Input parameter")
axs[1, 1].legend()

fig.tight_layout()
plt.show()

# Save the cross-validation data
df = pd.DataFrame({"Input parameters" : input_params.flatten(), "Predicted parameters" : predicted_params.flatten()})
df.to_csv("CrossValidation_FinalPredictions.csv")
...
def AnalyzeCurves():
    ...
    print(np.max(error))
    plt.hist(np.sort(error)[:9999], bins = 100)
    ...
    for i in range(2):
        for j in range(3):
            ...
            if(i == 1):
                axis[i][j].set_xlabel("Time t(s)")

            if(j == 0):
                axis[i][j].set_ylabel("$ C_{t} (s^{-1}) $")
...
def Plot_AIF_Ct():
    ...
    plt.plot(tnew, AIFnew, label='CA concentration in the artery AIF')
    plt.plot(tnew, Ct, label='CA concentration in the kidney $C_t$')
    plt.title(f'AIF and CA concentrations for $ (K^{{trans}} (1/s), T_g (s), v_p, \Delta (s))$ = {ToftsParams}')
    plt.xlabel('Time t (s)')
    plt.ylabel('Concentration $ (s^{-1}) $')
    plt.legend()
    plt.show()
